...
void init_weight(Matrix* w, unsigned nneurones_prev)
{
    for (unsigned r = 0; r < w->get_rows(); ++r) {
        for (unsigned c = 0; c < w->get_cols(); ++c) {
            (*w)(r, c) = normalRand(0, 1.0 / sqrt(nneurones_prev));
        }
    }
...
Ann::Ann(double alpha, unsigned minibatch_size, unsigned number_of_layers, unsigned* nneurons_per_layer) : alpha(alpha), minibatch_size(minibatch_size), number_of_layers(number_of_layers), nneurons_per_layer(nneurons_per_layer), layers(number_of_layers){

    layers[0] = Layer(0, nneurons_per_layer[0], minibatch_size, minibatch_size)
    for (unsigned l = 1; l < number_of_layers; ++l) {
        layers[l] = Layer(l, nneurons_per_layer[l], nneurons_per_layer[l-1], minibatch_size);

    }
}

Layer::Layer(unsigned layer_number, unsigned number_of_neurons, unsigned nneurons_previous_layer, unsigned minibatch_size) 
    : minibatch_size(minibatch_size),
      number_of_neurons(number_of_neurons),
      // On initialise les objets directement ici :
...
Layer::print_layer()
{
    ...
    print_matrix(z, true);
    printf(">> Activations --\n");
    print_matrix(activations, true);
    
    printf(">> Weights --\n");
    print_matrix(weights, true);
    printf(">> Biases --\n");
    print_matrix(biases, true);

    printf(">> Delta --\n");
    print_matrix(delta, true);
...
Ann::print_nn()
{
    ...
    {
        printf("Layer %d ", l);
        print_layer(layers[l]);
    }
}



Ann::forward(func_id_t activation_type)
{
    for (int l = 1; l < number_of_layers; l++)
    {
        ...
        cur.z2_tmp = cur.biases * cur.one_row;                      // z2 = b^l x [1,1,...,1]
        cur.z = cur.z2_tmp + cur.z1_tmp                             // z^l = z1 + z2
        cur.activations = cur.z.apply_function(activation_type);    // a^l = sigmoid(z^l)

    }
}

void Ann::backward(Matrix& y, func_id_t derive_activation_type)
{
    unsigned L = number_of_layers - 1;


    layers[L].delta = layers[L].activations - y;                                // (a^L - y)
    layers[L].dfz   = layers[L].z.apply_function(derive_activation_type);       // f'(z^L)
    layers[L].delta = layers[L].delta.hadamard(layers[L].dfz);                  // delta^L = (a^L - y) o f'(z^L)
    ...
    for (unsigned l = L; l > 0; --l)
    {
        ...
        prev.dfz      = prev.z.apply_function(derive_activation_type); // f'(z^(l-1))
        prev.delta    = cur.delta_tmp.hadamard(prev.dfz); // delta^(l-1) = (W^l)^T x delta^l o f'(z^(l-1))
    }
    

    double learning_rate_factor = alpha / minibatch_size;
    ...
    for (unsigned l = 1; l < number_of_layers; ++l)
    {
        ...
        cur.weights = cur.weights - (cur.w1 * learning_rate_factor); // W^l = W^l - (alpha/m) * Gradient


        cur.b1     = cur.delta * cur.one_col;            // delta^l x 1_col (Somme sur le batch)
        cur.biases = cur.biases - (cur.b1 * learning_rate_factor);   // b^l = b^l - (alpha/m) * Gradient
    }
}

Layer::~Layer() {
    // vide parce qu'on gère déjà bien la mémoire des sous objets
}
