// Build with the provided Makefile: `make`. See README.md for details.
...
double accuracy(image* test_img, byte* test_label, unsigned datasize, unsigned minibatch_size, ann_t *nn)
{
    ...
    for (int i = 0; i < datasize - minibatch_size; i+= minibatch_size)
    {
        ...
        forward(nn, sigmoid); // runs entirely on the GPU
        matrix_GPU_to_CPU(nn->layers[nn->number_of_layers-1]->activations); // bring the prediction back to host memory
...
int main(int argc, char *argv[])
{
    ...
    for (int epoch = 0; epoch < 1; epoch ++)
    {
        ...
        for (int i = 0; i < datasize - minibatch_size ; i+= minibatch_size)
        {
            ...
            // Stage the minibatch input on the GPU (once per minibatch).
            memcpy(nn->layers[0]->activations->m, x, 28 * 28 * minibatch_size * sizeof(double));
            matrix_CPU_to_GPU(nn->layers[0]->activations);

            // Stage the target labels on the GPU (once per minibatch).
            memcpy(out->m, y, 10 * minibatch_size * sizeof(double));
            matrix_CPU_to_GPU(out);

            // Forward + backward run entirely on the GPU; no further host<->device transfers.
            forward(nn, sigmoid);
